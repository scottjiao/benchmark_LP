...
prefix="baselineLP_newCsv";specified_args=["dataset",   "net","inProcessEmb",]


fixed_info={"task_property":prefix,"net":"myGAT","inProcessEmb":"True",}
task_space={"hidden-dim":"[64,128]","num-layers":"[3,4]","lr":"[1e-4,5e-4,1e-3,5e-3]","weight-decay":"[1e-4,5e-4,1e-3,5e-3]","feats-type":[3],"num-heads":[2,4],"epoch":[300],"decoder":["dot"],"batch-size":[8192]}
...
resources=gpus.split()
pool=multiprocessing.Queue(len(resources)*load)
for i in resources:
    for j in range(load):
        pool.put(i+str(j))
